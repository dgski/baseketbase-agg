...
(define (render-footer order start end posts-length)
  `(div ((class "footer"))
        (div ((class "controls"))
             ,(if [not (= start 0)]
                  `(a ((class "control-link") (href ,(string-append "?start=" (number->string (- start POSTS_PER_PAGE)) "&end=" (number->string (- end POSTS_PER_PAGE))))) "< prev") "     ")
             ,(if [>= posts-length POSTS_PER_PAGE]
                  `(a ((class "control-link") (href ,(string-append "?sort=" order "&start=" (number->string (+ start POSTS_PER_PAGE)) "&end=" (number->string (+ end POSTS_PER_PAGE))))) "next >") "     "))))
